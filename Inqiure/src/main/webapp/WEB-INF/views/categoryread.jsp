...
<form id="frm" method="post" action="categoryupdate">
	...
	$.ajax({
		...
		success:function(data) {
			if($(data.CATEGORYNAME)!= null){
				addList += "<thead><td width=100>대분류 상품 코드</td><td width=200>대분류 상품 이름</td></thead></tbody id=''body>";
				$(data).each(function() {
					var rowspan=3;
					addList += "<tr class='lcategory'><td width=100 class='lcategorycode'>"+this.CATEGORYCODE.split("/")[1]+"</td>";
					addList +=	"<td width=300 class='lcategoryname'><input type='text' size=30 class='largename' value='"+this.CATEGORYNAME.split("/")[1]+"'></td></tr>";
				});
					addList +="</tbody>";
				$("#tbl").html(addList);
			}
...
// 				$(data).each(function() {
// 					var rowspan=3;
// 					addList += "<tr class='row' code='"+this.CODE+"' name='"+this.NAME+"'><td width=100 class='lcategoryname' >"+this.NAME.split("/")[1]+"<input type='button' class='btnaddrow' value='+'</td>";
...
// 			$("#tbl1 > tbody:last").append(str);
	...
	$(frm).submit(function(e){
		...
		if(!$(frm).is(":hidden")){
			...
			var lcode="347-88-00867/"+strlcode+"/000";
			var strlname=$(this).find(".lcategoryname").find(".largename").val();
			var lname="에이치엘인베스트/"+strlname+"/empty";
